          ...
          <v-layout fill-height align-center justify-space-between>
            <v-flex>
              <v-layout row>
                <v-flex ma-2>
                  <v-text-field
                    v-bind:id="'section-title-field-' + tag_.id"
                    placeholder="タグ名"
                    single-line
                    outline
                    clearable
            ...
            <v-flex md3 xs3 sm3>
              <v-layout row>
                <v-flex xs4 sm2 md2 lg2 lx2>
                  <v-btn icon ripple @click.stop="deleteRow(tag_)">
...
export default class TagRow extends Vue {
  ...
  public deleteRow(tag: Tag): void {
    console.log("tag=" + tag.name);
  }

  private created(): void {
    if (this.tag_ == undefined) {
      throw new Error("tag prop undefine");
    }
  }
}
</script>
